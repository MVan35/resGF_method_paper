...
out <- '~/Desktop/SAMARCH/Paper1/submission/test/sim_single_cat'
...
eval_test_df <- NULL;
for (i in 1:iters) {
  # detach("package:AICcmodavg", unload=TRUE)
  # detach("package:cAIC4", unload=TRUE)
  library(adegenet)
  library(pegas)
  ...
  # locs0$group <- as.numeric(cut(locs0$locs, nb_pop))
  # create some "populations" 
  # get the average for each pop
  # locs0[order(locs0$group),]
  # pop_cords <- aggregate(locs0[, 1], list(locs0$group), mean)
  ...
  # dps.out <- mat_gen_dist(x = x, dist = "DPS")
  ...
  # create a categrorical landscape
  raster <- gaus
  values(raster)[values(raster) <= 0.3] <- 1 # crops
  values(raster)[values(raster) >= 0.3 & values(raster) < 0.60 ] <- 2 # grassland
  values(raster)[values(raster) >= 0.60 & values(raster) < 0.75 ] <- 3 # forest
  values(raster)[values(raster) >= 0.75 & values(raster) < 0.8 ] <- 4 # shrubland
  values(raster)[values(raster) >= 0.8 & values(raster) < 1 ] <- 5 # artificial areas
  raster2 <- raster
  plot(raster2)
  
  raster2 <- fbm
  values(raster2)[values(raster2) <= 0.3] <- 1 # crops
  values(raster2)[values(raster2) >= 0.3 & values(raster2) < 0.60 ] <- 2 # grassland
  values(raster2)[values(raster2) >= 0.60 & values(raster2) < 0.75 ] <- 3 # forest
  values(raster2)[values(raster2) >= 0.75 & values(raster2) < 0.8 ] <- 4 # shrubland
  values(raster2)[values(raster2) >= 0.8 & values(raster2) < 1 ] <- 5 # artificial areas
  raster2 <- raster2
  plot(raster2)
  
  
  values(combine_surface)[values(combine_surface) <= 0.3] <- 1 # crops
  values(combine_surface)[values(combine_surface) >= 0.3 & values(combine_surface) < 0.60 ] <- 2 # grassland
  values(combine_surface)[values(combine_surface) >= 0.60 & values(combine_surface) < 0.75 ] <- 3 # forest
  values(combine_surface)[values(combine_surface) >= 0.75 & values(combine_surface) < 0.8 ] <- 4 # shrubland
  values(combine_surface)[values(combine_surface) >= 0.8 & values(combine_surface) < 1 ] <- 5 # artificial areas
  plot(combine_surface)

  mystack <- stack(raster, raster2, combine_surface)
  names(mystack) <- c("gaus", "fbm", "combine_surface")
  plot(mystack)
  gaus <- mystack$gaus
  fbm <- mystack$fbm
  combine_surface <- mystack$combine_surface
  ...
  ## second models
  gaus_gf <- resGF(gf, gaus, save.image = T, results_dir = out_scenario)
  gaus_imp <- get_importance(gf, gaus)
  ...
  comb_surface_imp <- get_importance(gf2, combine_surface)
  ...
  fbm_imp <- get_importance(gf3, fbm)
  ...
  topAICc <-rownames(myAICc)[1]
  # mycAIC4 <- eval_df2[order(eval_df2["cAIC4"]),]
  # topmycAIC4<-rownames(mycAIC4)[1]
  ...
  # mycontrib_list <- list('raster1', 'raster2', 'raster2', 'raster4', 'raster5', 'raster6', 'raster7', 'raster8', 'raster9', 'raster10')
